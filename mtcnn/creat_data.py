        ...
        for i, line in enumerate(open(anno_src)):
            ...
            try:
                ...
                with Image.open(image_file) as img:
                    ...
                    for _ in range(5):
                        # 让人脸中心点有少许的偏移
                        w_ = np.random.randint(-w * 0.2, w * 0.2)/2
                        h_ = np.random.randint(-h * 0.3, h * 0.3)/2
                        cx_ = cx + w_
                        cy_ = cy + h_

                        # 让人脸形成正方形，并且让坐标也有少许的偏离
                        side_len = np.random.randint(int(min(w, h) * 0.8), np.ceil(min(w*1.25, h)))
                        ...
                        iou = ways.iou(crop_box, np.array(boxes))[0]
                        if iou > 0.7:  # 正样本
                    ...
                    #
                    # for i in range(6):
                    #     side_len = np.random.randint(face_size, face_size*1.5)
                    ...
                    #
                    #     if np.max(ways.iou(crop_box, _boxes)) <0.01:
                    #         face_crop = img.crop(crop_box)
