...
class Detector():
    def __init__(self,P_net_param="./module/pnet.pt",
                 ...
                 O_net_param="./module/onet1.pt",isCuda=True):
        self.isCuda=isCuda
    ...
    def _P_net_detect(self,image):
        ...
        while min_side_len > 12:
            ...
            x=torch.gt(cls, 0.7)
            idxs = torch.nonzero(x)
            cls = cls[x].detach().numpy()
        ...
        return ways.NMS(boxes, 0.5, FT=False)
    ...
    def O_net_detect(self,image,Rnet_boxes):
        ...
        for _box in _Rnet_boxes:
            ...
            px1= _x1+ow*offset[idxs][:,4]
            py1=_y1+ oh*offset[idxs][:, 5]
            px5 = _x2+ow * offset[idxs][:, 12]
            py5 =_y2+ oh*offset[idxs][:, 13]

            boxes = np.array([x1, y1, x2, y2, cls,px1,py1,px5,py5]).T
        return ways.NMS(np.array(boxes),0.2,FT=True)

if __name__ == '__main__':

    image_file = r"G:\图片\3.jpg"
    detector = Detector()
    ...
    with Image.open(image_file) as im:
        ...
        for box in boxes:
            ...
            # py5=int(box[8])
            # print(x1,px1,x2)
            imDraw.rectangle((x1, y1, x2, y2), outline='red',width=4)
            # imDraw.rectangle((px1, py1,px5, py5), outline='red', width=4)
        im.show()